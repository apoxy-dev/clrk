...
import type { AgentEgressRow, AgentEventRow, RevisionRow } from './agent-detail-data'
...
export function AgentDetailView({
  ...
  const tabs: Array<{ id: TabId; label: string; icon: CarbonIconType; count?: number }> = [
    headTab,
    { id: 'revisions', label: 'Revisions', icon: Version, count: revisions.length },
    { id: 'egress', label: 'Egress', icon: NetworkIcon, count: egress.length },
    { id: 'events', label: 'Events', icon: Archive, count: events.length },
  ]

  const status = row.ready ? (isTask ? 'Ready' : row.phase || 'Running') : row.readyReason || 'Not ready'
  ...
  return (
    <div className="agent-detail">
        ...
        <div className="page-head-l">
          <div className="page-head-titles">
            ...
            <div className="meta">
              ...
              <span className="dot-sep">·</span>
              <span style={{ fontFamily: 'var(--font-mono)' }}>{shortImage(row.image)}</span>
              <span className="dot-sep">·</span>
              <span>
              ...
              {!isTask && row.upSince && (
                <>
                  <span className="dot-sep">·</span>
                  <span>
                    up <b style={{ color: 'var(--text-primary)' }}>{row.upSince}</b>
                  </span>
                  <span className="dot-sep">·</span>
                  <span>
      ...
      {tab === 'egress' && <EgressTab egress={egress} onOpenGateway={onOpenGateway} />}
...
function InteractionTab({ spans, loading, initialId }: { spans: Span[]; loading?: boolean; initialId?: string }) {
  ...
  if (!inv) {
    return (
      <div className="viz-frame viz-empty-state">
        {loading ? 'Loading invocations…' : 'No invocations recorded for this agent yet.'}
      </div>
    )
  }
  ...
  return (
    <>
      <InvocationStrip invocations={invs} selectedId={inv.id} onSelect={(id) => { setInvId(id); setSpanId(null) }} />
      <div className="stat-strip" style={strip(6)}>
        <Stat lab="Status" val={String(inv.statusCode)} unit={inv.ok ? 'OK' : 'ERR'} tone={inv.ok ? 'ok' : 'err'} />
        <Stat lab="Duration" val={fmtMs(inv.durMs)} />
        <Stat lab="LLM tokens in / out" val={fmtK(inv.tokIn)} unit={`/ ${fmtK(inv.tokOut)}`} />
      ...
      <div className="viz-frame">
        <LaneChart
          ...
          items={inv.spans.filter((s) => s.lane !== 'inbound').map((s) => chartItem(s, s.t0Ms))}
          rootBar={
            inv.inbound
              ? {
                  id: inv.inbound.id,
      ...
      {span && <SpanInspector span={span} traceId={inv.traceId} invocationId={inv.id} />}
    </>
  )
}
            ...
            >
              ...
              <div className="inv-pill-body">
                ...
                <div className="inv-pill-meta">
                  {fmtMs(i.durMs)} · {i.statusCode} · {fmtK(i.tokIn + i.tokOut)} tok
                </div>
              </div>
              <span className="inv-pill-when">{fmtAgo(now - i.startNano / 1e6)}</span>
            </button>
          ))}
        </div>
      </div>
...
function ActivityTab({ row, spans, loading }: { row: AgentRow; spans: Span[]; loading?: boolean }) {
  ...
  const windowSec = preset && 'sec' in preset ? preset.sec : Math.max(60, maxAgo * 1.04)
  ...
  useEffect(() => {
    if (selId !== 'session' && !visible.some((c) => c.id === selId)) setSelId('session')
  }, [selId, visible])

  const selected =
    selId === 'session' ? 'session' : visible.find((c) => c.id === selId) ?? 'session'

  return (
    <>
      <div className="stat-strip" style={strip(6)}>
        <Stat lab="Phase" val={row.phase || 'Running'} tone={row.ready ? 'ok' : 'err'} />
        <Stat lab="Uptime" val={row.upSince ?? '—'} />
        <Stat lab="Restarts" val={String(row.restarts ?? 0)} />
        <Stat lab="LLM tokens in / out · 24h" val={metricStr(row.inTokens24h)} unit={`/ ${metricStr(row.outTokens24h)}`} />
        <Stat lab="Tool calls · 24h" val={metricStr(row.tools24h)} />
        <Stat lab="Errors · 24h" val={metricStr(row.errors24h)} tone={row.errors24h ? 'err' : undefined} />
      </div>
      <div className="viz-frame">
        ...
        {visible.length === 0 ? (
          <div className="dtrace-empty">
            {loading ? 'Loading captured egress…' : 'No calls captured in this window.'}
          </div>
        ) : (
          <LaneChart
            lanes={LANES_DAEMON}
    ...
    <div className="span-inspector">
      ...
      <div className="span-inspector-bd">
        <div className="span-main">
          ...
          <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.55, maxWidth: 640 }}>
            A DaemonAgent has no request boundary — it is not triggered, it just runs. Egress is
            still fully captured: each LLM, MCP, and network call above is its own span on the wall
            clock, with its own trace id, rather than nested under an invocation.
          </div>
        </div>
      </div>
    </div>
...
interface LaneDef { id: string; label: string; sub: string }
interface ChartItem {
  id: string
  laneId: string
  t0Ms: number
...
interface RootBar { id: string; laneId: string; x0Ms: number; x1Ms: number; label: string; error: boolean }
...
function LaneChart({
  ...
  const tickClass = (t: number) => {
    const p = pct(t)
    return 'swim-tick' + (p <= 0.5 ? ' swim-tick--first' : p >= 99.5 ? ' swim-tick--last' : '')
  }
    ...
    <div className="swim">
      <div className="swim-axis">
        ...
        <div className="swim-axis-track">
          {ticks.map((t) => (
            <span key={t} className={tickClass(t)} style={{ left: `${pct(t)}%` }}>
              {fmtTick(t)}
            </span>
          ))}
        </div>
          ...
          return (
            <div key={lane.id} className="swim-lane" style={{ height: laneHeight(pack.rows) }}>
              ...
              <div className="swim-lane-track">
                {ticks.map((t) => (
                  <span key={`g${t}`} className="swim-grid" style={{ left: `${pct(t)}%` }} />
                ))}
                {root && (
                  <SwimRootBar
                    bar={root}
...
function SwimRootBar({
  ...
  return (
    <button
      type="button"
      className={'swim-bar swim-bar--root' + (bar.error ? ' is-err' : '') + (selected ? ' is-sel' : '')}
      style={{ left: `${left}%`, width: `${width}%`, cursor: onSelect ? 'pointer' : 'default' }}
...
function SwimSpanBar({
  ...
  const fg = item.ok && item.lane === 'net' ? 'var(--text-primary)' : 'var(--apx-bone)'
  return (
    <button
      type="button"
...
function Legend({ kind }: { kind: 'task' | 'daemon' }) {
  return (
    <div className="swim-legend">
      ...
      <span className="lg-item"><span className="lg-sw lg-sw--llm" />LLM call</span>
      <span className="lg-item"><span className="lg-sw lg-sw--mcp" />MCP tool</span>
      <span className="lg-item"><span className="lg-sw lg-sw--net" />Network</span>
      <span className="lg-item"><span className="lg-sw lg-sw--err" />Error / 5xx</span>
      <span style={{ marginLeft: 'auto', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
...
function laneHeading(lane: Lane): string {
  return lane === 'inbound' ? 'Request' : lane === 'llm' ? 'GenAI call' : lane === 'mcp' ? 'MCP tool call' : 'HTTP call'
}

function curatedRows(s: Span): Array<[string, ReactNode]> {
  if (s.lane === 'llm') {
    return [
      ['gen_ai.system', s.provider ?? '—'],
      ['gen_ai.request.model', s.model ?? '—'],
      ['gen_ai.response.stream', s.stream ? 'true' : 'false'],
      ['gen_ai.usage.input_tokens', fmtK(s.tokIn ?? 0)],
      ['gen_ai.usage.output_tokens', fmtK(s.tokOut ?? 0)],
    ]
  }
  if (s.lane === 'mcp') {
    return [
      ['mcp.server', s.server ?? '—'],
      ['mcp.tool.name', s.tool ?? '—'],
      ['mcp.method', s.method ?? '—'],
      ['host', s.host || '—'],
    ]
  }
...
function attrRows(s: Span, traceId: string, invocationId?: string): Array<[string, ReactNode]> {
  ...
  rows.push([
    'invocation.id',
    invocationId ? invocationId : <span style={{ color: 'var(--text-muted)' }}>none · daemon span</span>,
  ])
  return rows
}
    ...
    <div className="token-bar">
      <div className="token-bar-row">
        <div className="token-bar-bg">
          <div className="token-bar-fill" style={{ width: `${(tokIn / total) * 100}%` }} />
        </div>
      ...
      <div className="token-bar-row">
        <div className="token-bar-bg">
          <div className="token-bar-fill token-bar-fill--out" style={{ width: `${(tokOut / total) * 100}%` }} />
        </div>
        <div className="token-bar-lab">
          out <b>{fmtK(tokOut)}</b>
        </div>
...
function RevisionsTab({ revisions }: { revisions: RevisionRow[] }) {
  if (revisions.length === 0) {
    return <div className="viz-frame viz-empty-state">No sandbox revisions for this agent yet.</div>
  }
  return (
    <div className="viz-frame">
          ...
          {revisions.map((r) => (
            <tr key={r.name}>
              ...
              <td style={{ fontFamily: 'var(--font-mono)' }}>
                {r.name}
                {r.active && <span className="chip chip--ink" style={{ marginLeft: 8 }}>active</span>}
              </td>
              <td style={{ fontFamily: 'var(--font-mono)', fontSize: 14 }}>{shortImage(r.image)}</td>
              <td>
                <span className={r.ready ? 'chip chip--leaf' : 'chip chip--coral'}>{r.ready ? 'Ready' : 'Not ready'}</span>
              </td>
              <td style={{ fontFamily: 'var(--font-mono)' }}>{r.readyWorkers}</td>
              <td style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{r.age}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function EgressTab({ egress, onOpenGateway }: { egress: AgentEgressRow[]; onOpenGateway?: (name: string) => void }) {
  if (egress.length === 0) {
    return <div className="viz-frame viz-empty-state">This agent references no egress gateways.</div>
  }
  return (
    <div className="viz-frame">
      <table className="ovw-table">
        ...
        <tbody>
          {egress.map((g) => (
            <tr
              ...
              onClick={g.exists && onOpenGateway ? () => onOpenGateway(g.name) : undefined}
            >
              <td>
                <span className={'gws-pip' + (g.ready ? '' : ' err')} />
              </td>
              <td style={{ fontFamily: 'var(--font-mono)', fontWeight: 500, color: 'var(--apx-blue-deep)' }}>{g.name}</td>
              <td style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{g.namespace}</td>
              <td style={{ fontFamily: 'var(--font-mono)' }}>{g.exists ? g.listeners : '—'}</td>
              <td>
                <span className={g.ready ? 'chip chip--leaf' : 'chip chip--coral'}>
                  {g.exists ? (g.ready ? 'Ready' : g.statusReason || 'Degraded') : 'Not found'}
                </span>
              </td>
    ...
    <div className="viz-frame ovw-events">
      {events.map((e, i) => (
        <div key={i} className="ovw-event">
          ...
          <div className={'ovw-event-rail' + (e.tone === 'error' ? ' ovw-event-rail--error' : e.tone === 'warn' ? ' ovw-event-rail--warn' : '')} />
          <div className="ovw-event-body">
            <div className="ovw-event-line">
              <span className={e.tone === 'error' ? 'chip chip--coral' : e.tone === 'warn' ? 'chip chip--amber' : 'chip'}>{e.type}</span>
            </div>
...
function Stat({ lab, val, unit, tone }: { lab: string; val: string; unit?: string; tone?: 'ok' | 'err' }) {
  const color = tone === 'ok' ? 'var(--apx-leaf)' : tone === 'err' ? 'var(--apx-coral)' : undefined
  return (
    <div className="stat">
...
function laneFill(lane: Lane, ok: boolean): string {
  ...
  return lane === 'inbound' ? 'var(--apx-ink)' : lane === 'llm' ? 'var(--apx-blue)' : lane === 'mcp' ? 'var(--apx-graphite)' : 'var(--apx-stone)'
}
function laneStroke(lane: Lane, ok: boolean): string {
  if (!ok) return 'var(--apx-coral)'
  return lane === 'inbound' ? 'var(--apx-ink)' : lane === 'llm' ? 'var(--apx-blue-deep)' : lane === 'mcp' ? 'var(--apx-ink-2)' : 'var(--apx-graphite)'
}
