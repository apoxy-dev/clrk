...
func TestInboundRoundtrip(t *testing.T) {
	...
	cgroupPath, err := InitWorkerCgroup()
	if err != nil {
		t.Skipf("worker cgroup v2 unavailable (%v) — needs a delegated cgroup v2 host", err)
	}
	...
	mgr := NewManager(ManagerConfig{
		...
		RootDir:          t.TempDir(),
		LogsDir:          t.TempDir(),
		PodName:          "inbound-roundtrip-test",
		ImageStore:       NewImageStore(t.TempDir()),
		IMDSHostAddr:     "127.0.0.1:18092", // unused by this workload, but Create wants them set
		EgressHostAddr:   "127.0.0.1:18093",
		WorkerCgroupPath: cgroupPath,
	})
	...
	inst, err := mgr.Create(ctx, CreateRequest{
		ID:       id,
		AgentRef: "resident-test",
		Identity: proxyproto.AgentIdentity{Namespace: "default", Name: "resident-test"},
		Sandbox: clrkv1alpha1.AgentSandbox{
			Image:   imageRef,
			Command: []string{"/server", residentServeArg, listenAddr},
		},
		// The drive-seam: must be set on the request so Create seals it into
		// the InitStr before the Sentry boots. (APO-694 2a.)
	...
	if inst.InboundSockPath == "" {
		t.Fatal("InboundSockPath empty after Start")
	}
	...
	client := &http.Client{
		...
		Transport: &http.Transport{
			DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) {
				return (&net.Dialer{}).DialContext(ctx, "unix", inst.InboundSockPath)
			},
		},
	}
