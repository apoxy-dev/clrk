...
func bootstrapControllerManager(ctx context.Context, cluster *drivers.ClusterDriver, image, pullPolicy, otelEndpoint, gatewayIP string) error {
	...
	// no-op (k3s side is upstream kube-apiserver so this is mostly
	// belt-and-suspenders — the real win is the matching check on the
	// WorkerPool below, which goes through the aggregated apiserver)
	// surfaces as a hard error instead of a hung pod with stale env.
	return drivers.ApplyAndVerify(ctx, cluster, deploy, verifyCMDeployment(image, len(deploy.Spec.Template.Spec.Containers[0].Env)))
}

// verifyCMDeployment returns a comparator that checks the cm Deployment
// the apiserver stored matches what we sent on the fields that matter:
// the container image (so an out-of-band patch swapping the image gets
// reclaimed by our ForceOwnership) and the env count (so a stripped
// PodSpec — the exact failure mode seen on WorkerPool — is detectable
// here too).
...
func bootstrapDefaultWorkerPool(ctx context.Context, cluster *drivers.ClusterDriver, image string, replicas int32, pullPolicy string) error {
	...
	if err := drivers.ApplyAndVerify(ctx, cluster, wp, verifyWorkerPool(image)); err != nil {
...
// aggregated apiserver stored matches what we sent. We assert image
// (the original bug: a stripped spec persisted clrk/worker:latest
// across reapplies) and the privileged SecurityContext (the same
// stripped-spec failure mode silently demoted the worker out of
// privileged, breaking runsc gofer fork). One indicative field per
// failure mode — full deep-equal would tilt at status/managedFields.
