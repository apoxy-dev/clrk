...
// the exact reverse of installTCPForwarder. fd is the host AF_UNIX listening
// socket the worker handed to the Sentry (PreInit → FilePayload →
// InitStackArgs.FDs); listenAddr is the in-sandbox "ip:port" the resident
// server (workerd, or a spike stub) is listening on.
//
...
// dial reaches the resident listener — not the egress catch-all forwarder
// installed in doInit — because a bound listening endpoint shadows the global
// TCP transport handler (proven in inbound_demux_test.go).
//
...
// dial toward the resident server. Mirror of handleTCP: there, the sandbox
// side is the forwarder-created endpoint and the upstream is a host dial;
// here, the host side is the accepted conn and the "upstream" is the in-stack
// dial into the resident listener.
func (s *Stack) handleInbound(hostConn net.Conn, target tcpip.FullAddress, proto tcpip.NetworkProtocolNumber) {
	...
	if err != nil {
		...
		// finished listen()ing yet — the readiness gate is the worker's
