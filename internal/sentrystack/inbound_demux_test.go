...
//	catch-all tcp.NewForwarder that sentrystack installs via
...
// same way installTCPForwarder does.
package sentrystack
...
// worker ships to the Sentry. It is cross-platform (pure JSON, no gvisor), so
// it runs on the dev host. The omitempty behavior matters for compatibility:
// an egress-only sandbox must encode no inbound keys so the payload stays
...
// robust spike target since lo exists even in Phase 1 lo-only mode) and
// installs the catch-all egress forwarder exactly as installTCPForwarder does.
// The returned counter records how many TCP SYNs reached the forwarder (i.e.
// matched no listening endpoint).
func newProofStack(t *testing.T) (*stack.Stack, *atomic.Int64) {
	...
	// Catch-all forwarder, installed exactly as installTCPForwarder does. The
	// handler records the hit and RSTs — standing in for the egress dial path
	// (routedDialer) so we can observe whether a SYN that should reach the
	// resident listener instead fell through to the forwarder.
