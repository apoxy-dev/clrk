...
// splices it to a fresh host net.Dial("unix", hostSocketPath) toward the
...
//   - No fd donation. The Sentry process shares the host mount + net namespaces,
//     so it net.Dial("unix", hostSocketPath)s the host control socket directly;
//     nothing rides cmd.ExtraFiles. That is why control is sealed entirely in the
//     init payload with no InboundFDIndex analogue.
//
...
func (s *Stack) installControlForwarder(listenAddr, hostSocketPath string) error {
	...
	go s.acceptControl(ln, hostSocketPath)
	return nil
}
...
func (s *Stack) acceptControl(ln net.Listener, hostSocketPath string) {
	for {
		...
		go func() {
			...
			s.handleControl(guestConn, hostSocketPath)
		}()
	}
}

// handleControl splices one guest-originated control connection to a fresh host
// dial of the manager's AF_UNIX control socket. Mirror of handleInbound: there
// the host side is the accepted conn and the in-stack dial is the upstream; here
// the in-stack accept is the guest side and the host unix dial is the upstream.
func (s *Stack) handleControl(guestConn net.Conn, hostSocketPath string) {
	...
	logger := slog.With(
		...
		slog.String("host_socket", hostSocketPath),
	)

	hostConn, err := (&net.Dialer{}).DialContext(ctx, "unix", hostSocketPath)
	if err != nil {
		...
		logger.Warn("Failed to dial host control socket", slog.Any("error", err))
		return
	}
